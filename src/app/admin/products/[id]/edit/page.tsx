    ...
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
          ...
          pricePerRose: String(p.pricePerRose || ""),
          bouquetPrice: String(p.bouquetPrice || ""),
          originalPricePerRose: String(p.originalPricePerRose || ""),
          originalBouquetPrice: String(p.originalBouquetPrice || ""),
          discount: String(p.discount || 0),
          category: p.category || "Roses",
          inStock: String(p.inStock ?? true),
        });
        if (p.image) {
          setImagePreview(`${process.env.NEXT_PUBLIC_API_BASE_URL}${p.image}`);
        }
      } catch (err: any) {
